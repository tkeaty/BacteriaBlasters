...
class Level2: SKScene {
    ...
    var invincible: Bool
    let bacteriaMovePPS: CGFloat = 380.0
    var lives = 3
    var gameOver = false
    ...
    override func update(_ currentTime: TimeInterval) {
        if currentTime - lastSpawn > 6 && bacteriaCount < bLimit && spawnCount < round{
        ...
        if spawnCount == round && bacteriaCount == 0 && !gameOver{
            ...
            let transitionScene = LevelTransition3(size: size)
    ...
    override init(size: CGSize){
        ...
        for i in 1...5 {
            textures.append(SKTexture(imageNamed: "Bacteria4D\(i)"))
        }
        ...
        textures.append(SKTexture(imageNamed: "Bacteria4"))
        for i in 1...2{
            textures.append(SKTexture(imageNamed: "Bacteria4M\(i)"))
            dextures.append(SKTexture(imageNamed: "Bacteria4K\(i)"))
        }
        dextures.append(SKTexture(imageNamed: "Bacteria4K3"))
            ...
            if bact.name == "childBacteria"{
                ...
                if bact.zPosition != 51{
                    for bullet in bullets{
                        if bullet.frame.intersects(bact.frame.insetBy(dx: 0.50*bwidth, dy: 0.50*bheight)){
                ...
                for plasmid in plasmids{
                    if plasmid.frame.intersects(bact.frame.insetBy(dx: 0.50*bwidth, dy: 0.50*bheight)) && bact.zPosition != 51{
                        resist(bacteria: bact)
                        plasmid.run(SKAction.sequence([SKAction.scale(to: 0.0, duration: 0.2),SKAction.removeFromParent()]))
                    }
                }
                
                if player.frame.intersects(bact.frame.insetBy(dx: 0.50*bwidth, dy: 0.50*bheight)) && !invincible{
    ...
    func shot(sprite: SKSpriteNode){
        ...
        if hp > 50{
            ...
            let pix: [SKTexture] = [SKTexture(imageNamed: "Bacteria4"),SKTexture(imageNamed: "Bacteria4K1"),SKTexture(imageNamed: "Bacteria4")]
    ...
    func moveBacteria(){
        ...
        for bact in bacteriaArr{
            ...
            if randomNum == 3 && (bacteriaSpawnTime[index!] + 5.0) < lastUpdateTime{
                ...
                if bact.zPosition == 51{
                    movePoint = normVect*425.0*CGFloat(self.dt)
                }
