...
class GameScene: SKScene {
    ...
    var invincible: Bool
    let bacteriaMovePPS: CGFloat = 250.0
    var lives = 3
    var gameOver = false
    ...
    override func didMove(to view: SKView) {
        ...
        rpad.position = CGPoint(x: screenSize.width - 200, y: 200)//player.position + CGPoint(x: 500, y: -200)
        dpad.position = CGPoint(x: 200, y: 200)//player.position + CGPoint(x: -500, y: -200)
    ...
    override func update(_ currentTime: TimeInterval) {
        if currentTime - lastSpawn > 3 && bacteriaCount < bLimit && spawnCount < round{
        ...
        if spawnCount == round && bacteriaCount == 0 && !gameOver{
            ...
            let transitionScene = LevelTransition2(size: size)
    ...
    override init(size: CGSize){
        ...
        for i in 1...5 {
            textures.append(SKTexture(imageNamed: "Bacteria1D\(i)"))
        }
        ...
        textures = [SKTexture(imageNamed: "Bacteria1")]
        var dextures: [SKTexture] = []
        for i in 1...2{
            textures.append(SKTexture(imageNamed: "Bacteria1M\(i)"))
            dextures.append(SKTexture(imageNamed: "Bacteria1K\(i)"))
        }
        dextures.append(SKTexture(imageNamed: "Bacteria1K3"))
            ...
            if bact.name == "childBacteria"{
                ...
                if bact.zPosition != 51{
                    for bullet in bullets{
                        if bullet.frame.intersects(bact.frame.insetBy(dx: 0.3*bwidth, dy: 0.3*bheight)){
                ...
                for plasmid in plasmids{
                    if plasmid.frame.intersects(bact.frame.insetBy(dx: 0.3*bwidth, dy: 0.3*bheight)) && bact.zPosition != 51{
                        resist(bacteria: bact)
                        plasmid.run(SKAction.sequence([SKAction.scale(to: 0.0, duration: 0.2),SKAction.removeFromParent()]))
                    }
                }
                
                if player.frame.intersects(bact.frame.insetBy(dx: 0.3*bwidth, dy: 0.3*bheight)) && !invincible{
    ...
    func shot(sprite: SKSpriteNode){
        ...
        if hp > 50{
            ...
            let pix: [SKTexture] = [SKTexture(imageNamed: "Bacteria1"),SKTexture(imageNamed: "Bacteria1K1"),SKTexture(imageNamed: "Bacteria1")]
    ...
    func moveBacteria(){
        ...
        for bact in bacteriaArr{
            ...
            if randomNum == 3 && (bacteriaSpawnTime[index!] + 5.0) < lastUpdateTime{
                ...
                if bact.zPosition == 51{
                    movePoint = normVect*400.0*CGFloat(self.dt)
                }
